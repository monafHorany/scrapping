...
async function getProductsData(link, urls = []) {
  try {
    ...
    const wrapper = dataTable("#productLists .product-item_productListItem__t_UMA");
    for (let i = 0; i < wrapper.length; i++) {
      const element = wrapper[i];
      const productUrl = dataTable(element)
        .find(".product-item_root__6ZVGl .product-item_image__IxD4T a")
        .attr("href");
