...
async function getProductsData(link, urls = []) {
  try {
    ...
    const wrapper = dataTable("#product-list-container > div.showcase-container > div > div");
    for (let i = 0; i < wrapper.length; i++) {
      const element = wrapper[i];
      const productUrl = dataTable(element).find("div > div.showcase-content > div.showcase-title > a").attr("href");
