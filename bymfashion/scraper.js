...
async function getProductsData(link, urls = []) {
  try {
    ...
    const dataTable = cheerio(
      "div.row div.col-6.col-sm-6.col-md-4.col-lg-4.col-xl-4.mb-2.product-item",
      outerhtml
    );
    ...
    dataTable.each(function () {
      const productUrl =
        cheerio(this)
          .find(
            "div.w-100.h-100.d-flex.flex-direction-column.justify-content-between.bg-white.ease div.w-100.position-relative a"
          )
          .attr("href");
    ...
    if (link.includes("ps=")) {
      ...
      if (+pageNumber > 1) {
        urls = urls.slice(9 * (+pageNumber - 1));
      }
    }
    // return await getProduct(urls[1], link);
