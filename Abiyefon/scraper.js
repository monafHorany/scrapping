...
const url = "https://www.abiyefon.com/abiye-ayakkabi";
...
async function getProduct(prodURL, link, productImage, productsizes) {
  try {
    ...
    const description = "Dekolte:" + cheerio("#tab1 > div > div", html).text();
    ...
    let images = [];
    const image = cheerio(
      "#contents > div > section > div.product-section > div.product-image > div.thumbimages.ps-container > div > div > ul",
      html
    ).html();
    $(image)?.each(function (i, elm) {
      images?.push($("li img", elm).data("src"));
    });
